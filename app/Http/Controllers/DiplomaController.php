...
use App\Mail\ConfirmacionPagoEmail;
...
class DiplomaController extends Controller
{
    ...
    public static function buscarCarnet($carnet)
    {
    ...
    public function enviarConfirmacionPago(Request $request)
    {
        $participanteId = $request->input('participante_id');
        $participante = Participante::find($participanteId);

        if ($participante) {
            Mail::to($participante->email)->send(new ConfirmacionPagoEmail($participante));
            return redirect()->back()->with('success', 'Confirmación de pago enviada con éxito.');
        } else {
            return redirect()->back()->withErrors(['No se encontró ningún participante con ese ID.']);
        }
    }
}
